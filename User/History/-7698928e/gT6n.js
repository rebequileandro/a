...
export const slice = createSlice({
  ...
  reducers: {
    ...
    sendFloorBar: (state, action) => {
      state.danceFloorBar.selected = action.payload;
    },
    resetStatesCheckout: (state) => {
      state.danceFloorBar = initialState.danceFloorBar;
    },
    clearSelectedFloor: (state) => {
      state.danceFloorBar.selected = initialState.danceFloorBar.selected;
    },
    ...
    setErrordanceFloorBar: (state, action) => {
      state.danceFloorBar.error = action.payload;
    }
  }
});
...
export const getAllDanceFloor = (id) => async (dispatch) => {
  try {
    ...
    dispatch(setErrordanceFloorBar(null))
    dispatch(setBar(response.data.data));
  } catch (error) {
    console.log(error);
    dispatch(clearCheckout())
    dispatch(setErrordanceFloorBar(error.response.data))
  }
};
...
export const getErrorDanceFloorBar = (state) => state.partyUser.checkout.danceFloorBar.error;
