...
export const GradientGreenBar = ({
  ...
  // console.log('cashOrder pa', cashOrder);
    ...
    if (method?.name === 'points') {
      ...
      try {
        ...
        socket.emit('cliente:pagocorrecto', {
          ...
          messagePush: response.data.data.orderPayment.map(
            (e) =>
      ...
      try {
        const response = await axios.post(
          `${REACT_APP_API}/partyuser/payment/mercadopago/sendorder`,
          {
            ...
            accessTokenMercadopago: currentClub.accessTokenMercadopago,
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        socket.emit('cliente:pagocorrecto', {
          id: response?.data.data._id,
          titlePush: 'Nuevo pedido',
          messagePush: getCart.map(
            (item) =>
              `${item.nameDrink[0].toUpperCase() + item.nameDrink.slice(1)} x${
                item.amount
              }`
          )
        });
          ...
          if (response.status === 200) {
            socket.emit('cliente:pagocorrecto', {
              ...
              messagePush: getCart.map(
                (item) =>
                  `${
                    item.nameDrink[0].toUpperCase() + item.nameDrink.slice(1)
                  } x${item.amount}`
              )
            });
            socket.emit('cliente:pagado', {
  ...
  return (
    <>
      ...
      <StatusPopUp
        ...
        redirect={
          status?.redirect ? status.redirect :status.status === 200 ?  redirectStatusPopup : status.status === 300 ? redirectLoaderPopup
        }
