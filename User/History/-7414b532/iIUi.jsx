...
const Bar_Page = () => {
  ...
  const [bars, setBars] = useState(initialStateBars);
  ...
  const handleButton = ({ image, id, nameBar, nameFloor }) => {
    ...
    setBarSelected({ bar: nameBar, floor: nameFloor });
  };

  //Si todo esta en orden, envía la información de la barra seleccionada a checkout para proceder con el pago y la orden, sino envia una alerta
  const handleCLick = () => {
    if (barSelected.bar && barSelected.floor) {
  ...
  return (
    <>
      ...
      />
      <div className="layout-primary bar-page">
        <h3 className="bar-page__title heading-tertiary-sub">
          Selecciona la zona y la barra por la que vas a retirar tu pedido.
        </h3>
        <section className="section-bar-floor">
          <div className="section-bar-floor__select--container">
            <Select
              placeholder={floor}
          ...
          {/* 
            dependiendo el piso elegido se despliega en forma de botones las opciones posibles y esto renderizaria la imagen de la barra justo abajo 
        */}
