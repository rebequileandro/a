  ...
  const handleSubmit = async (e) => {
    ...
    if (!loading) {
      ...
      if (
        ...
        setError({
          ...error,
          lastname: "Este campo debe tener al menos 3 caracteres",
        });
      ...
      } else if (input.company.length < 5) {
        setError({
          ...error,
          company: "Completa este campo",
        });
        ...
        try {
          const response = await axios.post(
            `${REACT_APP_API}/organizer/contact/new`,
            {
              name: input.name,
          ...
          await hubSpotForm(input);
          if (response.status === 200) {
            setInput(initialState);
            setDone(
              "Gracias por enviar el formulario. Nos pondremos en contacto lo más rápido posible."
            );
            setTimeout(() => {
              cancel();
            }, 4500);
