    ...
    getMyDrinks: (state, action) => {
      action.payload.map((e) => {
        ...
        if (e.typeDrink === 'additional') {
          let exists = state.additional.filter((i) => i._id === e._id);
          let index = state.additional.findIndex((j) => j._id === e._id);
          if (exists) {
...
export const getTeam = (id) => async (dispatch) => {
  try {
    const response = await axios.post(`${REACT_APP_API}/organizer/barra/team/all`, {
      idParty: id
    });
