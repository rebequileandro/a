    ...
    getMyDrinks: (state, action) => {
      action.payload[0]?.drinkParty.map((e) => {
        ...
        if (e.typeDrink === 'additional') {
          let exists = state.additional.filter(
            (i) => i.nameDrink === e.nameDrink
          );
          let index = state.additional.findIndex(
            (j) => j.nameDrink === e.nameDrink
          );
          if (exists) {
...
export const editParty = (party, id) => async () => {
  try {
    const response = await axios.put(`${REACT_APP_API}/organizer/party/${id}`, party);
    return response;
  } catch (error) {
    console.log(error);
    return error
...
export const upDateDrinks = (id, update) => async (dispatch) => {
  try {
    const response = await axios.put(
      `${REACT_APP_API}/organizer/drink/${id}`,
      update
    );
...
export const getTeam = (id) => async (dispatch) => {
  try {
    const response = await axios.post(`${REACT_APP_API}/organizer/barra/all`, {
      idParty: id
    });
...
export const newBottle = (data, id) => async (dispatch) => {
  try {
    const response = await axios.post(
      `${REACT_APP_API}/organizer/inventory/add`,
      data
    );
