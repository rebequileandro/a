...
export const orderTimeout = 150; //600;
const initialState = [];
const slice = createSlice({
  ...
  reducers: {
    ...
    updateTimeLeft: (state, action) => {
      ...
      const index = state.findIndex((ord) => ord.id === id);
      let newState = [...state];
      newState[index] = {
        ...newState[index],
        timeLeft: timeLeft
  ...
  try {
    ...
    response.data.data.map((e) => {
      ...
      if (!checkorder.length) {
        ...
        console.log('soy time left del dispatch', timeLeft);
        dispatch(
          initOrder({
            id: e._id,
            timestamp: timestamp,
        ...
        e.orderStages === ORDER_STATUS.ORDER_DEVERED ||
        e.orderDelivered === 'true'
      ) {
        dispatch(deliverOrder(e._id));
