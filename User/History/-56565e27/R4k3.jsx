...
export const DinamincCard = ({input}) => {
    ...
    if(input.cardNumber.length === 2){
        ...
        if(input.cardNumber[0].concat(input.cardNumber[1]) === '34' || 
           input.cardNumber[0].concat(input.cardNumber[1]) === '37'){
            console.log(parseInt(input.cardNumber[0].concat(input.cardNumber[1])))
            setTimeout(() => {
