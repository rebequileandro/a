  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const responseTeam = await axios.post(
        `${process.env.REACT_APP_API}/global/team/login`,
        {
          email: email,
          password: password
        }
      );
      console.log('partyUser response', responsePartyUser);
      console.log('owner response', responseOwner);
      console.log('team response', responseTeam.data);

      if (responsePartyUser.data.status === 'success') {
        setLoading(false);
        dispatch(logInUser(responsePartyUser.data.data)).then(() =>
          window.location.reload()
        );
      } else if (responsePartyUser.data.status === 'bad request') {
        setInputErrors({
          ...inputErrors,
          password: responsePartyUser.data.message
        });
        setLoading(false);
      } else if (responseOwner.data.status === 'success') {
        setLoading(false);
        dispatch(logInUser(responseOwner.data.data));
      } else if (responseOwner.data.status === 'bad request') {
        setInputErrors({ ...inputErrors, password: responseOwner.message });
        setLoading(false);
      } else if (responseTeam.data.status === 'success') {
        setLoading(false);
        dispatch(logInUser(responseTeam.data.data));
      } else if (responseTeam.data.status === 'bad request') {
