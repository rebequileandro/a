...
const Support = () => {
  ...
  const [input, setInput] = useState({
    ...
    email: user.email,
    telefono:
      user.phone?.length && `${user.phone[0].prefix}${user.phone[0].number}`,
    asunto: '',
    mensaje: '',
    urgencyfilter: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    input.mensaje &&
      input.mensaje.split(/\s+/).join('').length < 100 &&
      (newErrors.mensaje =
        'Mensaje demasiado corto, debe tener al menos 100 caracteres');
  ...
  return (
    <>
      ...
      <StatusPopUp
        ...
        description={
          status.status === 200
            ? 'Te redirigiremos a un chat para que puedas contarnos con más detalle tu problema'
            : null
        }
        status={status.status === 200 ? true : false}
        redirect={() =>
          status.status === 200 ? window.open(status.data) : setStatus(false)
        }
        button={'Aceptar'}
      />
    </>
