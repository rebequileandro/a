...
export const slice = createSlice({
  ...
  reducers: {
    ...
    updateDelivered: (state, action) => {
      const { status, id } = action.payload;
      let order = state.allOrders.find((e) => e._id === id);
      if (order) order.orderDelivered = status;
    }
  }
});
...
export const getOrderBartender = (id) => async (dispatch) => {
  try {
    ...
    if (
      response.data.data.payment.orderStages === ORDER_STATUS.ORDER_DELIVERED
    ) {
      dispatch(setStatus('Las bebidas ya fueron entregadas'));
    } else if (
      response.data.data.payment.orderStages === ORDER_STATUS.PAYMENT_PENDING
    ) {
...
export const deliverOrder = (id, barra) => async (dispatch) => {
  try {
    ...
    (id, ORDER_STATUS.ORDER_DELIVERED);
