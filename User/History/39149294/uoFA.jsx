...
export const Categories = ({ title, category, setCategoryType }) => {
  const navigate = useNavigate();
  const handleOnClick = () => {
    if (title === "promociones") {
      setCategoryType("promotions");
    } else {
      setCategoryType(category[0].typeDrink);
    }
    navigate("/marketplace/section");
  };
          ...
          <div className="products">
            {category?.map((e) => {
              if (JSON.parse(e.activeDrink) === true)
                return (
                  <div key={e.nameDrink} className="product-card">
                    <Card
                      add={true}
                      image={e.imageDrink}
                      name={e.nameDrink}
