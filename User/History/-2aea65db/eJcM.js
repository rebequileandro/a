...
export const formatDateISO = (date) => {
    ...
    let x = [...parseDate]
    x[1] = x[1]?.toString()?.padStart(2, '0');
    x[2] = x[2]?.toString()?.padStart(2, '0');
    parseDate = x.join('-')
    console.log(typeof parseInt(date[date.length - 1]) === "number")
    if (parseDate && parseDate[parseDate.length - 1] > 0) {
