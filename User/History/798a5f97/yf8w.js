...
export const getMainPageEvents = () => async (dispatch) => {
  try {
    const response = await axios(`${REACT_APP_API}api/partyuser/party/all`);
    dispatch(setMainPageEvents(response.data));
  } catch (error) {
    console.log(error);
  }
};
...
export const getMainPageEventsDiscount = () => async (dispatch) => {
  try {
    const response = await axios(`${REACT_APP_API}/partyuser/party/discountparties`);
