      ...
      if (!validation1 && !validation2) {
        ...
        try {
          const cardToken = await axios.post(
            ...
            {
              headers: {
                'api-key': process.env.REACT_APP_MOBBEX_PUBLIC_KEY
              }
            }
          );
          console.log('Token', cardToken);
          const response = await axios.post(
            `${process.env.REACT_APP_API}/partyuser/payment/mobbex/add`,
            {
              idUser: user._id,
          ...
          if (response === 200) {
            setStatuspopup({
              title: 'Listo',
              description:
                'Ya registramos tu tarjeta, ahora la podrás utilizar en todas tus compras.',
              status: 200
            });
          }
        } catch (error) {
