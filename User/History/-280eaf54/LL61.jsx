...
const ClinicalSimulator = () => {
  ...
  }, [chatRef.current, dataChat.length]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isLoading && message) {
      const previusState = message;
      setMessage("");
      const result = await sendMessage({ sender: "user", message: message });
      if (result?.error) {
        setMessage(previusState);
      }
    }
  };
  return (
    <div className="clinical-simulator">
      {dataChat.length ? (
        <div
          className="clinical-simulator__chat-container scroll-bar container-primary "
          ref={chatRef}
        >
          {dataChat?.map((item) => (
            <ChatMessage
      ...
      <form className="clinical-simulator__chat-input" onSubmit={handleSubmit}>
        <TextArea
