...
const EditRole = ({ role }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (role === 'newCashier' || role === 'newBartender') {
      if (!input.name || !input.name || !input.square) {
        setStatus({
          data: {
            title: 'Error',
        ...
        dispatch(
          teamParty({
            name: input.name,
            email: removeSpaces(input.email).toLowerCase(),
            password: removeSpaces(input.email).toLowerCase(),
      ...
      let data;
      if (removeSpaces(input.email).toLowerCase() === userRole.email) {
        data = {
          name: input.name,
          square: input.square
        };
      } else {
        data = { ...input, email: removeSpaces(input.email).toLowerCase() };
      }
  ...
  console.log(removeSpaces(input.email).toLowerCase());
  return (
    <>
      <Header
