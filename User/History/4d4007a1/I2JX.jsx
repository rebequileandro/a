...
const EditRole = ({ role }) => {
    ...
    if (role === 'newCashier' || role === 'newBartender') {
      if (!input.name || !input.name || !input.square) {
        ...
        dispatch(
          teamParty({
            name: input.name,
            email: removeSpaces(input.email),
            password: removeSpaces(input.email),
      ...
      let data;
      if (removeSpaces(input.email) === userRole.email) {
        data = {
          name: input.name,
          square: input.square
        };
      } else {
        data = { ...input, email: removeSpaces(input.email) };
      }
  ...
  console.log(removeSpaces(input.email));
  return (
    <>
      <Header
