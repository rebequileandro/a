            ...
            action.payload.forEach(e => {
                if (!state.inventory.length) {
                    state.inventory = [{
                        ...
                        brand: [{
                            ...
                            inventorySquare: e?.inventorySquare
                        }]
                    }]
                }
                state.inventory.forEach((j, i) => {
                    if (j.drink === e.name) {
                        ...
                        if (isExists.length) {
                            let newValue = [...state.inventory]
                            let indexbrand = state.inventory[i].brand.findIndex(index => index.id === e._id)
                            newValue[i].brand[indexbrand].amount = e.inventoryGeneral 
                            newValue[i].brand[indexbrand].inventorySquare = e.inventorySquare
                            state.inventory = newValue
                        }
                    }
                    let isExists = state.inventory.filter(f => f.drink === e.name)
                    if(!isExists.length) {
                        state.inventory = [
                            ...state.inventory,
                            {
...
export const  getAllMyParty = (id) => async (dispatch) => {
    const response = await axios.post(`${ REACT_APP_API}/organizador/party/all`, id)
...
export const  postNewParty = (formData) => async () => {
    try {
        const response = await axios({
             method: "post",
             url: `${ REACT_APP_API}/organizador/party/add`,
             data: formData,
...
export const  getbyId = (id) => async (dispatch) => {
    const response = await axios.get(`${ REACT_APP_API}/organizador/party/${id}`)
...
export const teamParty = (user) => async () => {
    try {
        const response = await axios.post(`${ REACT_APP_API}/register/cashier`, user)
        return response;
    } catch (error) {
        console.log(error)
    }
...
export const getTeam = (id) => async (dispatch) => {
    try {
        const response = await axios.post(`${ REACT_APP_API}/organizador/party/allteam`, {idParty: id})
...
export const updateTeam = (id, data) => async () =>{
    try {
        const response = await axios.put(`${ REACT_APP_API}/cashier/edit/${id}`, data)
        return response;
...
export const updateBottle = (id, data) => async (dispatch) => {
    try {
        const response = await axios.put(`${REACT_APP_API}/inventory/updatebottle/${id}`, data)
        dispatch(getInventory({idParty: response.data.idParty}))
        return response
    } catch (error) {
        return error
...
export const getStatisticsTotal = (data) => async (dispatch) => {
    try {
        const response = await axios.post(`${REACT_APP_API}/statistics/numerospedidototalorganizador`, data)
...
export const getStatisticsHistory = (id) => async (dispatch) => {
    try {
        const response = await axios.post(`${REACT_APP_API}/statistics/ventahistorialorganizador`, id)
...
export const getStatisticsTop5 = (id) => async (dispatch) => {
    try {
        const response = await axios.post(`${REACT_APP_API}/statistics/productosmasvendidosorganizador`, id)
