...
export const SelectDrink = ({selected, setSelected, setStatus, setIsOpenStatusPopup, id}) => {
    ...
    useEffect(() => {
      ...
      if(next) {
        setCreatePackData(getAllAdditional)
      } else {
    ...
    const handleSubmit = (e) => {
      ...
      selected === 'sin alcohol' ? allData = [getAllBottles , getAllPacks, getAllDrinks, drinks] :
      selected === 'packs' && (allData = [getAllDrinks, getAllBottles, getAllPacks, getAllAdditional])
        allData.map(element => {
          element.map(e => {
            newData.push(e)
            })
  ...
  return (
    ...
    <div className='popup-select-drink-overlay'>
      <div className='popup-select-drink'>
          ...
          <p>selecciona {selected === 'botellas' ? 'las' : selected === 'sin alcohol' ? 'las bebidas' : 'los'} {selected} disponibles en tu menu</p>
          ...
          <div className='add-drink-buttons'>
            {newPack.imageDrink.length ? ( !next ?
            <button className='save' onClick={() => setNext(true)}>Siguiente</button>
            :
            <button className='save' onClick={() => setIsCreate(true)}>Crear Pack</button>)
