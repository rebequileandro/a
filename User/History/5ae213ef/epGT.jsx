  ...
  const handleClickPopup = () => {
    if (status === TABLE_STAGES.CONFIRMED || status === TABLE_STAGES.READ) {
      ...
      dispatch(
        sendNotification({
          id: idClientePayment,
          title: 'Tu pedido ha sido comandado',
          message: 'Pronto estará listo',
          idParty: getUser.idParty,
          idOrder: id,
          orderStage: TABLE_STAGES.COMMANDED
        })
      );
      ...
      dispatch(
        sendNotification({
          ...
          message: 'Pronto te llevaremos tu pedido',
          idParty: getUser.idParty,
          idOrder: id,
          orderStage: TABLE_STAGES.DELIVERED
        })
      );
    }
      ...
      <div className="item-container">
        ...
        >
          ...
          {drink.length ? (
            <>
              <div className="item-container__comments-container">
                <h2 className="title-recipe-item__drinkName">Barra:</h2>
              </div>
              {drink?.map((e, i) => (
                <React.Fragment key={i}>
                  <div className="items-details-container__row">
                    ...
                    <div className="title-recipe-item">
                      <h3 className="title-recipe-item__drinkName">
                        {e.title}
                      </h3>
                    </div>
          ...
          {food.length ? (
            <>
              <div className="item-container__comments-container">
                <h2 className="title-recipe-item__drinkName">Cocina:</h2>
              </div>
              {food?.map((e, i) => (
                <React.Fragment key={i}>
                  <div className="items-details-container__row">
