...
import { createCalendar, formatDateISO } from "../../utils/format-date";
...
const Calendar = ({ year, onClick, months, dates }) => {
  ...
  const dat = new Date("2022-06-11").toISOString();
  console.log(dat.includes(new Date("2022-06-11T00:00:00.000Z").toISOString()));
  console.log(dat);
  return (
    <div className="calendar" title={`Calendario Bizarrap Tour ${year}`}>
      {calendar?.map((e, i) => (
        ...
        >
          ...
          {e.days?.map((day, d) => (
            <div
              className="calendar__day-wrapper"
              key={
                `${year}-${`0${i + 1}`.slice(-2)}-${`0${day}`.slice(-2)}` + d
              }
            >
              {dates?.includes(
                formatDateISO(
                  `${year}-${`0${i + 1}`.slice(-2)}-${`0${day}`.slice(-2)}`
                )
              ) && (
                <img
