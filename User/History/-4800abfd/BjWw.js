...
describe('Activities', () => {
    ...
    describe('Input Search', () => {
        const result = searchActivitie('pruebas', false, data)
        test('should return an array', () => {
            expect(Array.isArray(result)).toBeTruthy()
        });
        test('accept strings', () => {
            expect(searchActivitie(2, false, data)).toBeFalsy();
            expect(result.length ? true : false).toBeTruthy();
        });
        test('does not accept booleans', () => {
            expect(searchActivitie(true, false, data)).toBeFalsy();
            expect(searchActivitie(false, false, data)).toBeFalsy();
        });
    });
});