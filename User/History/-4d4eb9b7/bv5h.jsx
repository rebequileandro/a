...
import closeIcon from '../../../assets/buttons/close_icon.svg';
...
const OrderCardSimple = ({ order, showDetails, index, type, ordersLoaded }) => {
  ...
  if (order?.nameTable) {
    if (
      ...
      order.tableStage === 'NO_PAYMENT'
    ) {
      content = {
        title: '¡Sigue divirtiéndote!',
        description: (
          <p className="description">
            Pronto te llevaremos el pedido a la
            <span className="description__highlight"> {order?.nameTable}</span>
          </p>
        ),
        stageText: 'Tu pago ha sido confirmado',
        body: <OrderStages order={order} />
      };
    } else if (order.tableStage === TABLE_STAGES.COMMANDED) {
      content = {
        title: '¡Sigue divirtiéndote!',
        description: (
          <p className="description">
        ...
        stageText: 'Tu pedido ha sido comandado',
        body: <OrderStages order={order} />
      };
    } else if (order.tableStage === TABLE_STAGES.DELIVERED) {
      content = {
        ...
        stageText: '¡Tu pedido está listo!',
        body: <OrderStages order={order} />
      };
    }
  } else {
    if (order.status === ORDER_STATUS.PAYMENT_PENDING) {
      content = {
        ...
        description: (
          <p className="description">
            con el <span className="description__qrLink">QR</span> antes de que
            se <span className="description__cancelRed">cancele</span> a las{' '}
            <span className="description__expireTime">
              {expireTimeFormat} h
            </span>
          </p>
        ),
      ...
      content = {
        ...
        body: (
          <div className="order-delivered">
            ...
            <p className="order-delivered__enjoyTxt">¡Qué lo disfrutes!</p>
            {/* <button className="btn-primary--l qr-button-text">
              Repetir pedido
            </button> */}
          </div>
  ...
  return (
    <div className="link-order-card">
      <div className="order-card">
        ...
        order.tableStage === TABLE_STAGES.DELIVERED ? (
          <button
        ...
        {showDetails && (
          <div className="details">
            ...
            <p className="order-total">
              Total:{' '}
              <span>{formatPrice(order?.total, currentClub.currency)}</span>
            </p>
