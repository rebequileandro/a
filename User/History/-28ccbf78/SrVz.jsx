...
export const AddParty = () => {
  ...
  const handleSubmit = async (e) => {
      ...
      const formAddParty = new FormData();
        ...
        formAddParty.append("emailBarmans", JSON.stringify(input.emailBarmans))
        formAddParty.append("cashier", JSON.stringify(input.cashier))
      dispatch(postNewParty(formAddParty))
      .then((response) => {
          if (response.status === 200) {
            ...
            JSON.parse(response.data.emailBarmans)?.forEach(e => {
              dispatch(teamParty({
                name: e?.name,
                email: e?.email,
                password: e?.email,
  ...
  return (
    <>
   {error && <StatusPopUp button={"volver"} title={"ocurrió un error"} description={"inténtalo de nuevo, asegúrese de completar todos los campos"}/>}
