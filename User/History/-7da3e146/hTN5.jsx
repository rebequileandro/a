...
export const SocketReques = () => {
  ...
  useEffect(() => {
    ...
    socket.on("server:pagado", (data) => {
      dispatch(updateStatus({ id: data.id, status: ORDER_CONFIRMED }));
    ...
    socket.on("server:enpreparacion", (data) => {
      console.log("EN PREPARACIONNNNNNNNN", data);
      dispatch(updateStatus({id: data.id, status: IN_PREPARATION}));
    ...
    socket.on("server:confirmandoretiro", (data) => {
      dispatch(updateStatus({id: data.id, status: ORDER_READY}));
      fetch(`${REACT_APP_API}/webpush/new-message`, {
        method: 'POST',
        body: JSON.stringify({
