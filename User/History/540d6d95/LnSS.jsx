...
const Checkout = ({ socket }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const token = await registration.pushManager.getSubscription();

      // navigator.serviceWorker.updateSubscription({
      //   token,
      //   origin: window.location.osrigin,
      //   permissions: ['push', 'notification']
      // });
      console.log('notification token', token.getKey());
