...
const CalendarPicker = ({setIsOpen, action, setCurrentDate}) => {
    ...
    const handleClick = () => {
      ...
      if(startDate.day && startDate.month.number && startDate.year){
        setCurrentDate(startDate.day+'/'+(startDate.month?.number)+'/'+startDate.year.toString().slice(-2))
      }
      else{
        setCurrentDate(`${date.getDate()}/${date.getMonth()}/${date.getFullYear().toString().slice(-2)}`)
      }
      setIsOpen(false)
    }
