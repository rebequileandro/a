...
const CalendarPicker = ({setIsOpen, action, setCurrentDate, currentDate}) => {
    ...
    const handleClick = () => {
      ...
      if(startDate.day && startDate.month.number && startDate.year){
        setCurrentDate({
          ...
          initial: `${startDate.year}-${("0" + startDate.month?.number).toString().slice(-2)}-${("0" + startDate.day).toString().slice(-2)}`,
          end: endDate ? `${endDate.year}-${("0" + endDate.month?.number).toString().slice(-2)}-${("0" + endDate.day).toString().slice(-2)}` :
          `${startDate.year}-${("0" + startDate.month?.number).toString().slice(-2)}-${("0" + startDate.day).toString().slice(-2)}`
        })
      }
      else{
        setCurrentDate({
   ...
   console.log(`${startDate.year}-${("0" + (startDate.month?.number)).toString().slice(-2)}-${("0" + (startDate.day + 1)).toString().slice(-2)}`)
  return (
