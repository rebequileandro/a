 ...
 export default function reducer(state = initialState, action) {
     switch(action.type) {
        ...
        case 'SEARCH_DATA_RANGE':
            ...
            let format;
            const match = state.table.filter(e => e.date >= startDate.toJSON() && e.date <= endDate.toJSON())
            match.length && (format = match.filter.sort((a,b)=> new Date(a.date) < new Date(b.date) ? 1 : -1 ))
            console.log(format)
