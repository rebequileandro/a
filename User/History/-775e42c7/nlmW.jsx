...
const Party = () => {
    ...
    const handleClick = () => {
        window.location.href = `https://auth.mercadopago.com.ar/authorization?client_id=${REACT_APP_MP_CLIENT_ID}&response_type=code&platform_id=mp&redirect_uri=${REACT_APP_MP_REDIRECT}/${id}`
    }
  useEffect(() => {
    if(code){
      axios.post("https://api.mercadopago.com/oauth/token",
      {
        ...
        code: code,
        redirect_uri: `${REACT_APP_MP_REDIRECT}/${id}`,
      ...
      .then((result) => {
          //dispatch(editParty({accessToken: result.data.access_token}, id))
