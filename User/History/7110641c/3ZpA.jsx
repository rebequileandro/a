...
import {
  useLoginMutation,
  useSignupMutation,
} from "@/store/servicesSlice/auth.services";
import { useDispatch } from "react-redux";
import { setUser } from "@/store/slice/user.slice";
import { removeWhitespaces } from "@/utils/formValidation";
...
const Login = () => {
  ...
  const handleSubmitLogin = async (e) => {
    ...
    if (!isLoadingLogin) {
      ...
      if (!errors.email && !errors.password) {
        const formater = removeWhitespaces({ email: loginForm.email });
        const response = await login({ ...loginForm, ...formater });
        if (!isErrorLogin) {
  ...
  const handleSubmitSignUp = async (e) => {
    ...
    if (isVerified && !isLoadingSignup) {
      const isError = checkFiels(signupForm, 0, signupForm.length);
      if (!isError) {
        const fields = sliceObject(signupForm, 0, 5);
        const formater = removeWhitespaces(fields);
        const response = await signup({ ...signupForm, ...formater });
        if (!isErrorSignup) {
          dispatch(setUser(response?.data?.record));
        }
      }
