...
const Home = () => {
  ...
  const audioRecording = (e) => {
    e.preventDefault();
    if (e.button === 2) {
      // Si el botón derecho del ratón se presiona, establece el estado como verdadero
      setRec(true);

      // Aquí puedes realizar acciones al presionar el clic derecho
      console.log("Clic derecho presionado");
    }
  };
  const uploadAudio = (e) => {
    e.preventDefault();
    if (e.button === 2 && rec) {
      // Si el botón derecho del ratón se levanta y estaba presionado previamente, realiza acciones
      // Puedes hacer esto para capturar cuando se levanta el clic derecho después de ser presionado
      console.log("Clic derecho levantado");
    }

    // Restablece el estado del clic derecho
    setRec(false);
  };
  return (
    <div className="home">
