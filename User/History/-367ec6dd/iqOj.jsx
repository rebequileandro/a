...
export const OwnerForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (inputData.password !== inputData.repeatPassword) {
      ...
      try {
        ...
        setDone("Organizador registrado con éxito");
        form.reset();
        window.localStorage.setItem(
          "lastOrganizerName",
          response.data.data.name
        );
        window.localStorage.setItem("lastOrganizerID", response.data.data._id);
      } catch (error) {
        console.log(error);
      }
    }
  ...
  return (
    <article className="form-section">
      ...
      <br />
      <br />
      {window.localStorage.getItem("lastOrganizerName") && (
        <>
          <h3>Ultimo organizador registrado:</h3>
          <span>
            Nombre: {window.localStorage.getItem("lastOrganizerName")}
          </span>
        </>
      )}
      {window.localStorage.getItem("lastOrganizerID") && (
        <span>
          ID del organizador: {window.localStorage.getItem("lastOrganizerID")}
        </span>
      )}
    </article>
  );
