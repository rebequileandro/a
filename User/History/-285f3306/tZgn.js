  ...
  reducers: {
    ...
    removeNotification: (state, action) => {
      state.notifications = state.notifications.filter(
        (e) => e.idNotification !== action.payload
      );
    }
  }
});
...
export const sendNotification = (data) => async () => {
  try {
    const response = await axios.post(
      `${REACT_APP_API}/webpush/new-message`,
      data
    );
    return response;
  } catch (error) {
...
export const getNotifications = (data) => async (dispatch) => {
  try {
    const response = await axios.get(
      `${process.env.REACT_APP_API}/partyuser/notification/all/${data}`
    );
...
export const deleteNotifications = (data) => async (dispatch) => {
  ...
  try {
    await axios.delete(`${process.env.REACT_APP_API}/partyuser/notification`, {
      data: data
    });
  } catch (error) {
    console.log(error);
