  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (method.name === 'card') {
        ...
        console.log('RESPONSE PAYMENT CREDIT CARD', response);
        console.log('Id order', response.data._id);
        ...
        setPopupData({
          ...
          redirect:
            response.status === 200 ? () => navigate('/') : setPopupData(null),
          button: 'Aceptar'
        });

        if (response.data.status === 'approved') {
          console.log('atroden if');
