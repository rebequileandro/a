...
export const Scanner = () => {
  ...
  return (
    <div className='barman-container'>
        <Header welcome={true}/>
        {(data && !isOpen && getStatus === 200) ? <Ticket order={order} status={JSON.parse(order?.orderDelivered) === true && 'entregado'}/> : null}
        {getStatus !== 200 && getStatus ? <StatusPopUp title={getStatus} redirect={statusPopup} button={'aceptar'}/> : null}
        {isOpen && 
          <QrScanner 
            setData={setData} 
            setIsOpen={setIsOpen}/>}
