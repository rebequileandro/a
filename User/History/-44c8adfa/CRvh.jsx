...
export const EditBarmans = ({propsBarman, setPropsBarman, setIsEditBarman, indexInputbarmans, input, setInput, newBarman}) => {
    ...
    const saveChanges = (e) => {
      e.preventDefault()
      if(propsBarman.name.length >= 3 && propsBarman.email.length > 12){
        if(!newBarman) {
  ...
  return (
    <div className='container-edit-barmans'>
      <form className='container-form' onSubmit={(e) => saveChanges(e)}>
          ...
          <hr/>
          <button type='submit' className='save-changes'>Guardar cambios</button>
