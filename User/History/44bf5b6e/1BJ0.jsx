...
const MailAndNumber = () => {
  ...
  const [input, setInput] = useState({
    phone: {
      flag: user?.phone.length ? user.phone[0].flag : '🇦🇷',
      prefix: user?.phone.length ? user.phone[0].prefix : '+54',
      number: user?.phone.length ? user.phone[0].number : ''
    },
    email: user.email
  });
  ...
  const handleSubmit = (e) => {
    ...
    if (
      ...
      setLoading(true);
      dispatch(updateSettings(data, user._id || user.id)).then((response) => {
