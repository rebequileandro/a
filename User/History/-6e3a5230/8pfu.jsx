...
import { updateTableStage } from '../../../redux/slices/cashier/order';
const {
  ORDER_CONFIRMED,
  IN_PREPARATION,
  ORDER_READY,
...
export const SocketReques = ({ socket }) => {
  ...
  useEffect(() => {
    ...
    //socket food
    socket.on('server:comandado', (data) => {
      dispatch(
        updateTableStage({ status: TABLE_STAGES.COMMANDED, id: data.id })
      );
      navigator.vibrate(300);
    });
    socket.on('server:pedidoentregadocashier', (data) => {
      dispatch(
        updateTableStage({ status: TABLE_STAGES.DELIVERED, id: data.id })
      );
      navigator.vibrate(300);
    });
  }, []);
