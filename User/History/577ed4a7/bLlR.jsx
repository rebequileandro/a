...
const Orders = ({ socket }) => {
  ...
  const [selected, setSelected] = useState('Comandar');
  ...
  useEffect(() => {
    setAllOrdersArrayFiltered(
      allOrdersState
        ?.filter((order) => {
          return selected === 'Comandados'
            ? order?.tableStages === TABLE_STAGES.COMMANDED
            : selected === 'Entregados'
            ? order.tableStages === TABLE_STAGES.DELIVERED
            : selected === 'Comandar'
            ? order.tableStages === TABLE_STAGES.CONFIRMED ||
              order.tableStages === TABLE_STAGES.READ
            : null;
        })
        .filter(
          (order) =>
  ...
  return (
    <div className="orders-container layout-primary">
      ...
      <div className="orders-container__top-bar">
        ...
        <SearchBar input={inputSearchOrder} handleChange={handleSearchOrders} />
        <PrimarySwitch
      ...
      <div className="order-list-container">
        {allOrdersArrayFiltered?.length ? (
          ...
          <div className="without-orders">
            ...
            <p className="without-orders__text">No hay pedidos por Comandar.</p>
          </div>
        )}
      </div>
      <TabbarCashier />
