...
export default function MyAccount() {
  ...
  const handleLogout = async () => {
    let ordersPending = ordersBartender?.filter(
      (e) =>
        e.orderStages === ORDER_STATUS.IN_PREPARATION ||
        e.orderStages === ORDER_STATUS.GET_READY
    );
    if (orders.length) {
      orders.map((order) => {
    ...
    //   console.log(error);
    // }
    if (user.rol === 'bartender' && ordersPending.length) {
      setIsOpen(true);
    } else {
      document.cookie = '';
      localStorage.clear();
  ...
  return (
    <div className="my-account layout-primary">
      ...
      {isOpen && (
        <Popup_Alert
          title="Antes de cerrar sesión debes terminar los pedidos pendientes"
          button="Seleccionar"
          redirect={() => navigate('/')}
        />
      )}
