...
const NewRecap: React.FC<NewRecapIntefaceComponent> = (prop) => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!validation) {
      const response: any = await newRecap(inputState, { setProgress });
  ...
  useEffect(() => {
    formRef?.current.addEventListener("progress", (event: any) => {
      console.log(event);
      if (event.lengthComputable) {
        const porcentajeCargado = (event.loaded / event.total) * 100;
        progressBarRef.current.style.width = porcentajeCargado + "%";
      }
    });
  }, [formRef?.current]);
