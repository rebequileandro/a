...
const DeliveryMethods = () => {
  ...
  let selectOptions = cart.filter((product) => product.category === 'comida')
    .length
    ? ['A LA MESA']
    : ['POR BARRA', 'A LA MESA'];

  const [method, setMethod] = useState(
    selectOptions.length === 1 ? 'A LA MESA' : null
  );
  ...
  const handleCLick = () => {
    if (barSelected?.bar && barSelected?.floor && barSelected?.image) {
      let data = { ...barSelected };
      delete data.id;
      dispatch(sendFloorBar(data));
    ...
    <>
      ...
      <div className="layout-primary delivery-methods">
        <div className="delivery-methods__select delivery-methods__select--method">
          <Select
            initialState={method ? method : null}
