...
export default function ProductsCardEdit({
  image,
  name,
  price,
  oldPrice,
  ...
  const handleChangeSwitch = async (e) => {
    ...
    await axios.put(`${REACT_APP_API}/organizer/drink/${id}`, {
      activeDrink: !checked
    });
  };
  // ESTA FUNCION REVISA SI TIENE INGREDIENTES LA RECETA SINO LOS APAGA
  // useEffect(() => {
  //   async function checkRecipeIngredients() {
  //     const drinkHasRecipe = recipe?.every((obj) => obj.botella.length > 1);
  //     if (!drinkHasRecipe && statusDrink) {
  //       await axios.put(`${REACT_APP_API}/organizer/drink/${id}`, {
  //         activeDrink: false
  //       });
  //       setChecked(false);
