  ...
  const handleClick = async () => {
    ...
    if (action === "sendOrder") {
      ...
      if (method.name === "mercado pago") {
        ...
        //   console.log(error);
        // }
        const response = await axios.post(`https://api.mercadopago.com/v2/wallet_connect/agreements?client.id=${currentUser.id || currentUser._id}`,
        {
          return_url: "https://wedrinkapp.com/checkout",
    ...
    if (status.status === 200) {
      ...
      if (orderStatus === ORDER_STATUS.PAYMENT_PENDING) {
        intervalID = setInterval(() => {
          ...
          if (timeLeft >= 0) {
            dispatch(updateTimeLeft({ timeLeft, id: status.data._id }));
          } else if (timeLeft < 0) {
            dispatch(deliverOrder(status.data._id));
      ...
      navigate(`/mi-pedido/` + status.data._id);
    } else {
      setIsOpen(false);
    }
  };
